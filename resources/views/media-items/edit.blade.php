    ...
    <div class="flex-1 flex flex-col items-center justify-center p-4 sm:p-6 lg:p-8">
        ...
        <div
            ...
            <div
                ...
                <h1 class="text-xl font-bold text-slate-900 dark:text-white tracking-tight">Edit {{ ucfirst($mediaItem->type) }}</h1>
                <button
            ...
            <!-- Form Content -->
            <form class="p-6 md:p-8 space-y-6 flex-1 overflow-y-auto" action="{{ route('media-items.update', $mediaItem->id) }}"
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <div class="relative">
                        <input
                            ...
                            id="name" placeholder="e.g. One Piece" type="text" name="name" value="{{ $mediaItem->name }}" />
                    </div>
                    @error('name')
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="relative">
                            <select
                                ...
                                <option value="anime" {{ $mediaItem->type == 'anime' ? 'selected' : '' }}>Anime</option>
                                <option value="manga" {{ $mediaItem->type == 'manga' ? 'selected' : '' }}>Manga</option>
                                <option value="novel" {{ $mediaItem->type == 'novel' ? 'selected' : '' }}>Novel</option>
                            </select>
                        </div>
                        @error('type')
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <div class="relative">
                            <select
                                ...
                                <option value="plan" {{ $mediaItem->status == 'plan' ? 'selected' : '' }}>Plan to Watch</option>
                                <option value="watching" {{ $mediaItem->status == 'watching' ? 'selected' : '' }}>In Progress</option>
                                <option value="completed" {{ $mediaItem->status == 'completed' ? 'selected' : '' }}>Completed</option>
                                <option value="paused" {{ $mediaItem->status == 'paused' ? 'selected' : '' }}>Paused</option>
                                <option value="dropped" {{ $mediaItem->status == 'dropped' ? 'selected' : '' }}>Dropped</option>
                            </select>
                        </div>
                        @error('status')
                            <span class="text-red-500">{{ $message }}</span>
                    ...
                    <div class="flex items-center gap-3">
                        <div class="relative flex-1">
                            <input
                                ...
                                min="0" placeholder="0" type="number" name="progress_current" value="{{ $mediaItem->progress_current }}"/>
                            <div
                        ...
                        <div class="relative flex-1">
                            <input
                                class="form-input w-full rounded-lg text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-0 focus:ring-2 focus:ring-primary/50 focus:border-primary border border-slate-300 dark:border-input-border-dark bg-white dark:bg-input-bg-dark h-12 px-4 text-base font-normal transition-all"
                                placeholder="?" type="number" name="progress_total" value="{{ $mediaItem->progress_total }}"/>
                            <div
                                class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-600 text-xs font-semibold pointer-events-none">
                                TOTAL</div>
                        </div>
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <textarea
                        ...
                        id="notes" placeholder="Write your thoughts, reviews, or key moments here..." rows="4"
                        name="notes">{{ $mediaItem->notes }}</textarea>
                    @error('notes')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="flex items-center gap-3">
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="source" id="source"
                            class="form-textarea w-full resize-y rounded-lg text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-0 focus:ring-2 focus:ring-primary/50 focus:border-primary border border-slate-300 dark:border-input-border-dark bg-white dark:bg-input-bg-dark p-4 text-base font-normal leading-normal transition-all"
                            placeholder="Source of the media where consumed/consumes" value="{{ $mediaItem->source }}">
                    </div>
                    <div class="relative flex-1">
                        ...
                        <input type="text" name="source_url" id="source_url"
                            class="form-textarea w-full resize-y rounded-lg text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-0 focus:ring-2 focus:ring-primary/50 focus:border-primary border border-slate-300 dark:border-input-border-dark bg-white dark:bg-input-bg-dark p-4 text-base font-normal leading-normal transition-all"
                            placeholder="Source link of the media where consumed/consumes" value="{{ $mediaItem->source_url }}">
                    </div>
                    @error('source')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" name="image_url" id="image_url"
                        class="form-textarea w-full resize-y rounded-lg text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-0 focus:ring-2 focus:ring-primary/50 focus:border-primary border border-slate-300 dark:border-input-border-dark bg-white dark:bg-input-bg-dark p-4 text-base font-normal leading-normal transition-all"
                        placeholder="Image link for portrait" value="{{ $mediaItem->image_url }}">
                    @error('image_url')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror
                </div>
